...
export const SearchContextProvider = ({ children }: SearchContextProviderProps) => {
  ...
  const saveSearchValues = (destination: string, checkIn: Date, checkOut: Date, adultCount: number, childCount: number) => {
    ...
    sessionStorage.setItem("cheOut", checkOut.toISOString());
  ...
  return <SearchContext.Provider value={{ destination, checkIn, checkOut, adultCount, childCount, hotelId, saveSearchValues }}>{children}</SearchContext.Provider>;
};
